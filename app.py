...
model_file_path = "app/data/model/14--loss-0.1018--batch-4--time-18-01-02-52.pth"
pkl_path = "app/data/model/tmp_viz_obj.pkl"
    ...
    if request.method == 'POST':
        ...
        class Compiler:
            ...
            def compile(self, tokens, output_file_path):
                ...
                output_html = None # self.root.render(self.dsl_mapping, rendering_function=render_content_with_text)
                if output_html is None:
                    import pickle
                    from pathlib import Path
                    from html_transpiler.html_transpiler import HTMLTranspiler
                    with open(pkl_path, 'rb') as handle:
                        vis_obj=pickle.load(handle)
                    predictions = vis_obj["predictions"]
                    targets_filepaths = vis_obj["targets_filepaths"]
                    print(len(predictions))
                    print(len(targets_filepaths))
                    idx = 0
                    for filepath in targets_filepaths:
                        if filepath.name == img_name:
                            break
                        else:
                            idx = idx + 1
                    print(idx)
                    transpiler = HTMLTranspiler()
                    output_html = transpiler.transpile(predictions[40], insert_random_text=True)
                    return output_html
